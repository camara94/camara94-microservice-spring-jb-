...
@Component({
  selector: 'app-add-mention',
  templateUrl: './add-mention.component.html',
  styleUrls: ['./add-mention.component.scss']
})
export class AddMentionComponent implements OnInit {
  ...
  addMention(domaineId: number, mention: Mention) {
    setTimeout(() => {
      this.scrool = true;
      this.mentionService
          .createMentions(mention)
          .subscribe(data => {
            this.mention = data;
            this.mentionService
                .updateMentions(data.id, domaineId, data)
